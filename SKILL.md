---
name: graylight-audit
description: Graylight NewAPI 账单查询与火山主账户余额监控。当用户询问 NewAPI 用户余额、消费用量、充值记录、低余额用户、消费排名、channel 列表时使用；当询问火山账户余额或账单时使用；当需要生成查账日报、创建查账 cron job、推送消息到飞书或其他渠道时使用；当需要初始化 audit token 或管理 audit token 时使用。
---

# Graylight Audit Skill

Graylight 查账 skill。不提供固定日报脚本——OpenClaw 读取本文档后，按需调用通用请求脚本、聚合 JSON、生成交互答复或 cron 日报。

## Secrets

从 OpenClaw secrets 读取以下 key 并导出为环境变量后执行脚本：

| Key | 说明 |
|-----|------|
| `NEWAPI_BASE_URL` | NewAPI 服务地址，如 `https://ai.graylight.cn` |
| `NEWAPI_AUDIT_TOKEN` | 查账 token；缺失时读 [references/admin-setup.md](references/admin-setup.md) 执行 mint |
| `VOLC_ACCESS_KEY` | 火山只读 IAM 子账户 AK |
| `VOLC_SECRET_KEY` | 火山只读 IAM 子账户 SK |
| `VOLC_REGION` | 火山区域，默认 `cn-beijing` |

> Admin token 仅初始化时需要 → 见 [references/admin-setup.md](references/admin-setup.md)

## Pricing

Graylight 计价：**500000 quota = ¥1**，直接显示 ¥，无需换算美元。

```
¥ = quota / 500000
quota = ¥ * 500000
```

低余额阈值用 ¥，例如"低于 ¥10" → `quota < 10 * 500000 = 5000000`。

## NewAPI Audit API

**鉴权（所有查账请求）：**

```http
Authorization: Bearer ***<NEWAPI_AUDIT_TOKEN>
```

使用通用脚本：

```bash
NEWAPI_BASE_URL=https://ai.graylight.cn \
NEWAPI_AUDIT_TOKEN=*** \
bash scripts/newapi-request.sh GET '/api/user/?page_size=100'
```

### 接口列表

| 接口 | 参数 | 说明 |
|------|------|------|
| `GET /api/user/` | `p` `page_size`（建议100） | 所有用户，含 `quota` `used_quota` `request_count` |
| `GET /api/user/:id` | — | 单用户详情 |
| `GET /api/log/` | `type=2` `start_timestamp` `end_timestamp` `p` `page_size` `username` | 消费日志；按 `quota` 字段汇总 |
| `GET /api/user/topup` | `p` `page_size` | 充值历史 |
| `GET /api/channel/` | `p` `page_size` | channel 列表，含各 channel 余额 |

**分页：** 所有列表返回 `{success, data: {items: [...], total}}`；以 `total` 判断是否需要翻页。

## Volcengine Billing API

```bash
VOLC_ACCESS_KEY=*** VOLC_SECRET_KEY=*** \
python scripts/volc-billing.py QueryBalanceAcct

VOLC_ACCESS_KEY=*** VOLC_SECRET_KEY=*** \
python scripts/volc-billing.py ListBill '{"BillPeriod":"2026-05"}'
```

- `QueryBalanceAcct`：账户余额，返回 `Result.Balance` / `CashBalance` / `CreditBalance`
- `ListBill`：按月账单（可选，趋势分析）
- 失败时不中断 NewAPI 部分；在报告中标注错误原因

文档：https://www.volcengine.com/docs/6269

## Daily Report（Cron）

日报不是固定脚本，由 OpenClaw 按下方步骤组装执行。

**推荐 cron 配置：**

```yaml
name: graylight-daily-audit
schedule: "0 9,13,21 * * *"   # Asia/Shanghai
description: |
  Graylight 查账日报。低余额告警阈值：¥10（修改此处调整）。
  1. GET /api/user/?page_size=100 拉所有用户余额，翻页取完，找出低于阈值用户
  2. GET /api/log/?type=2&start_timestamp=<24h前>&end_timestamp=<now>&page_size=100 拉消费日志，翻页取完
  3. python scripts/volc-billing.py QueryBalanceAcct 拉火山余额
  4. 组装消息发送到 <目标会话/渠道>

  消息参考：
  📊 Graylight 查账日报 [时间]
  🏦 火山余额：¥XXXX（失败则标注原因）
  👥 用户总余额：¥XXXX | 24h消费：¥XXX（N次）
  🔝 24h消费TOP5：...
  ⚠️ 低余额用户（低于¥10）：...
```

消息目标（channel/target）在 cron 配置或调用时指定，不写死在 skill 里。

## Interactive Query Examples

**"user 17 上周用了多少"**
1. `GET /api/user/17` 拿 username
2. `GET /api/log/?type=2&start_timestamp=<上周一>&end_timestamp=<本周一>&username=<x>&page_size=100`，翻页取完
3. 汇总 `quota` → ¥

**"火山账户还有多少"**
→ `python scripts/volc-billing.py QueryBalanceAcct`，解析 `Result.Balance`

**"余额低于5元的用户"**
→ `GET /api/user/?page_size=100` 翻页取完，过滤 `quota < 5 * 500000`

**"本月消费最高的5个用户"**
→ 本月起始 timestamp 到 now，`GET /api/log/?type=2&...` 翻页取完，按 user 聚合 quota，降序 TOP5

## Scripts

| 脚本 | 说明 |
|------|------|
| `scripts/newapi-request.sh` | 通用 NewAPI 请求；`TOKEN_TYPE=admin` 切换 admin 鉴权 |
| `scripts/volc-billing.py` | 火山 Billing V4 签名请求；纯标准库无依赖 |
| `scripts/mint-audit-token.sh` | 初始化专用；见 [references/admin-setup.md](references/admin-setup.md) |

## Audit Token 失效恢复

查账返回 401/403 时 → 读 [references/admin-setup.md](references/admin-setup.md) 重新 mint，更新 `NEWAPI_AUDIT_TOKEN` secret。
